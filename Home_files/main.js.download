...
jQuery(document).ready(function () {
  ...
  if( parseInt(taa.is_home) != 1 && parseInt(taa.is_builder2) && !guest ) {
    tbdemo_blur_init();
  }
});

function tbdemo_blur_init() {
  let template = jQuery(document).find("#tbdemo-blurPage-template").html();
  jQuery("body").addClass("tbdemo-blur-active").append(template);
  jQuery(document).find("html", "body").css("overflow", "hidden");

  let header = jQuery(document).find("div[data-elementor-type='twbb_header']");
  let headerHeight = 0
  if( header.length != 0 ) {
    headerHeight = header.outerHeight();
  }
  let domainTopBar = jQuery(document).find(".twbb-topbar-domain");
  let domainTopBarHeight = 0
  if( domainTopBar.length != 0 ) {
    domainTopBarHeight = domainTopBar.outerHeight();
  }

  if( headerHeight || domainTopBarHeight ) {
    let top_space = parseInt(headerHeight + domainTopBarHeight);
    jQuery(document).find(".tbdemo-blur-layer").css("top", top_space + "px");
  }


  jQuery('.tbdemo-blur-button').on('click', function (e){
    if ( typeof dataLayer != "undefined" ) {
      dataLayer.push({
        'event': '10web-event',
        'event_name': 'Free Upgrade Offer',
        'tenweb_action': 'Locked button click',
        'tenweb_info': 'AI Builder Demo: Secondary page'
      });
    }
  });
}
